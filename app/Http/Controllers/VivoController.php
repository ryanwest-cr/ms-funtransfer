...
class VivoController extends Controller
{
	...
	public function authPlayer(Request $request)
	{
		...
		$response = '<?xml version="1.0" encoding="utf-8"?>';
			 		$response .= '<VGSSYSTEM><REQUEST><TOKEN>'.$request->token.'</TOKEN><HASH>'.$request->hash.'</HASH></REQUEST><TIME>12 Jan 2004 15:15:15</TIME><RESPONSE><RESULT>FAILED</RESULT><CODE>400</CODE></RESPONSE></VGSSYSTEM>';
		...
		else
		{
			if ($client_details) {
				...
				if(isset($client_response->playerdetailsresponse->status->code) 
					...
					header("Content-type: text/xml; charset=utf-8");
			 		$response = '<?xml version="1.0" encoding="utf-8"?>';
			 		$response .= '<VGSSYSTEM><REQUEST><TOKEN>'.$request->token.'</TOKEN><HASH>'.$request->hash.'</HASH></REQUEST><TIME>12 Jan 2004 15:15:15</TIME><RESPONSE><RESULT>OK</RESULT><USERID>'.$player_id.'</USERID><USERNAME>'.$client_response->playerdetailsresponse->accountname.'</USERNAME><FIRSTNAME></FIRSTNAME><LASTNAME></LASTNAME><EMAIL>'.$client_response->playerdetailsresponse->email.'</EMAIL><CURRENCY>USD</CURRENCY><BALANCE>'.$client_response->playerdetailsresponse->balance.'</BALANCE><GAMESESSIONID>785df376123dfg</GAMESESSIONID></RESPONSE></VGSSYSTEM>';
				}
				else
				{
		...
		Helper::saveLog('authentication', 5, file_get_contents("php://input"), $response);
		echo $response;
	...
	public function gameTransaction(Request $request) 
	{
		...
		$client_details = $this->_getClientDetails($client_code);
		$player_details = PlayerHelper::getPlayerDetails($request->userId);
		...
		else
		{
			...
			GameRound::create($request->roundId, $player_details->token_id);
				...
				$client = new Client([
						        	...
						        	[
									  ...
									  "fundtransferrequest" => [
										...
										"fundinfo" => [
										      ...
										      "rollback" => "false",
										      "currencycode" => $player_details->currency,
										      "amount" => ($transactiontype == 'BET' ? "-" : "").$request->Amount
										]
									  ]
									]
						    )]
					...
					&& $client_response->fundtransferresponse->status->code == "402") {
						...
						&& $client_response->fundtransferresponse->status->code == "200") {
								...
								GameTransaction::save(($transactiontype == 'BET' ? "debit" : "credit"), $json_data, $game_details, $client_details, $player_details);
