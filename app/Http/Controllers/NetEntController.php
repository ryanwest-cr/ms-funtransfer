...
class NetEntController extends Controller
{
	...
	public function withdraw(Request $request, $player){
		...
		// $client_details = NetEntHelper::getClientDetails('username',$player);
		...
		if($existing_bet != 'false'): // NO BET
			...
			NetEntHelper::saveLog('NetEnt Withdraw Idom', $this->provider_db_id,  json_encode($request->all()), json_encode($response));
			return json_encode($response);
		endif;
		...
		if ($player_details->playerdetailsresponse->balance < 0) {
			...
			NetEntHelper::saveLog('NetEnt Withdraw Balance', $this->provider_db_id,  json_encode($request->all()), json_encode($response));
			return json_encode($response);
		}
	...
	public function deposit(Request $request, $player){
		...
		$client_details = NetEntHelper::getClientDetails('token', $request["serverToken"]);
		$player_details = NetEntHelper::playerDetailsCall($client_details->player_token);
		NetEntHelper::saveLog('NetEnt Deposit', $this->provider_db_id, json_encode($request->all()), $response);
		$game_ext = NetEntHelper::checkTransactionExist($request['gameRoundRef'], 2); 
		if($game_ext == 'false'):
