...
class HabaneroController extends Controller
{
    ...
    public function fundtransferrequest(Request $request){
        ...
        if($player_details->playerdetailsresponse->balance < $bet_amount): // check balance
            $response = [
                "fundtransferresponse" => [
                    "status" => [
                        "success" => false,
        ...
        // $checkTrans = DB::table('game_transactions')->select('game_trans_id','bet_amount')->where('provider_trans_id','=',$provider_trans_id)->get();
        $checkTrans = DB::select("select game_trans_id, bet_amount from game_transactions where provider_trans_id = '".$provider_trans_id."' ");
        ...
        if(count($checkTrans) > 0): // transaction already exist (mostly realted to win)
            ...
            $checkT = DB::select("SELECT entry_id FROM game_transactions WHERE provider_trans_id='".$provider_trans_id."' and round_id= '".$round_id."' ");
                ...
                try{
                    ...
                    foreach($fundinfo as $funds):
                        if($funds->gamestatemode == 1){ //debit
                            ...
                            $updateGameTransExt = DB::table('game_transaction_ext')->where('game_trans_ext_id','=',$game_trans_ext)->update(["amount" => $bet_amount,"game_transaction_type" => "1","provider_request" => json_encode($details),"mw_response" => json_encode($temp_response),"mw_request" => json_encode($client_response->requestoclient),"client_response" => json_encode($client_response),"transaction_detail" => json_encode($temp_response) ]);
                        }else{ // credit
