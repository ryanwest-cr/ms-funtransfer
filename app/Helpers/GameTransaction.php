...
class GameTransaction
{
	...
	public static function solid_save($method, $request_data, $game_data, $client_data, $player_data) {
		/*var_dump($request_data); die();*/
		$trans_data = [
					"token_id" => $player_data->token_id,
					"game_id" => $game_data->game_id,
		...
		/*var_dump($trans_data); die();*/
		DB::table('solid_game_transactions')->insert($trans_data);
	}
	...
	public static function solid_update($method, $request_data, $game_data, $client_data, $player_data) {
        ...
        $update = DB::table('solid_game_transactions')
                ->where('game_trans_id', $game_details->game_trans_id)
                ->update(['pay_amount' => $pay_amount, 'income' => $income, 'win' => $win, 'entry_id' => $entry_id]);
	...
	public static function solid_find($original_trans_id) {
		$transaction_id = DB::table('solid_game_transactions')
								->where('provider_trans_id', $original_trans_id)
								->first();
	...
	public static function solid_bulk_rollback($round_id) {
		$transactions = DB::table('solid_game_transactions')
								->where('round_id', $round_id)
								->where('entry_id', 1)
								->get();
